...
class TodoControllerTest {
  ...
  void getAllTodos_returnsListOfTodos() {
    List<Todo> expected =
        List.of(todo("1", "Buy milk", "From the store", false), todo("2", "Walk dog", null, true));
    when(todoRepository.findAll()).thenReturn(expected);

    var body = assertThat(mockMvc.get().uri("/api/todos")).hasStatusOk().bodyJson();
  ...
  void getAllTodos_returnsEmptyList() {
    ...
    assertThat(mockMvc.get().uri("/api/todos")).hasStatusOk().bodyJson().isEqualTo("[]");
  }
  ...
  void getTodo_returnsExistingTodo() {
    ...
    var body = assertThat(mockMvc.get().uri("/api/todos/1")).hasStatusOk().bodyJson();
  ...
  void getTodo_returns404WhenNotFound() {
    ...
    assertThat(mockMvc.get().uri("/api/todos/missing")).hasStatus(404);
  }
  ...
  void updateTodo_returnsUpdatedTodo() throws Exception {
    when(todoRepository.findById("1"))
        .thenReturn(Optional.of(todo("1", "Buy milk", "From the store", false)));
    when(todoRepository.save(any(Todo.class))).thenAnswer(invocation -> invocation.getArgument(0));
  ...
  void completeTodo_returnsCompletedTodo() {
    ...
    var body = assertThat(mockMvc.patch().uri("/api/todos/1/complete")).hasStatusOk().bodyJson();
  ...
  void completeTodo_returns404WhenNotFound() {
    ...
    assertThat(mockMvc.patch().uri("/api/todos/missing/complete")).hasStatus(404);
  }
  ...
  void deleteTodo_returns204() {
    ...
    assertThat(mockMvc.delete().uri("/api/todos/1")).hasStatus(204);
  ...
  void deleteTodo_returns404WhenNotFound() {
    ...
    assertThat(mockMvc.delete().uri("/api/todos/missing")).hasStatus(404);
  }
}
